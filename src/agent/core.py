...
class LennyGrowthAgent:
    """Core conversational agent orchestrating RAG retrieval, LLM inference, citations, and artifacts."""
    def __init__(self, retriever: TranscriptRetriever, llm_client: Optional[BaseLLMClient] = None):
    ...
    def _extract_artifact(self, raw_text: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        """Detects and extracts HTML or Markdown code blocks for the Artifact Viewer."""
    ...
    async def chat(self, user_message: str, chat_history: Optional[List[dict]] = None, top_k: int = 4) -> AgentResponse:
        ...
        if not retrieval.is_grounded or not retrieval.chunks:
            return AgentResponse(
                ...
                citations=[], artifact_type=None, artifact_title=None, artifact_content=None,
                model_used=self.llm_client.model_name, is_grounded=False
            )

        context_xml = self._assemble_context(retrieval.chunks)
        history_context = ""
        if chat_history:
            history_lines = [f"{h.get('role', 'user').capitalize()}: {h.get('content', '').strip()}" for h in chat_history[-6:]]
            history_context = f"\n<conversation_history>\n" + "\n".join(history_lines) + "\n</conversation_history>\n"

        user_prompt = f"""<transcript_context>\n{context_xml}\n</transcript_context>\n{history_context}\n<user_query>\n{user_message}\n</user_query>"""
        raw_response = await self.llm_client.generate(system_prompt=AGENT_SYSTEM_PROMPT, user_prompt=user_prompt, temperature=0.3)
        art_type, art_title, art_content = self._extract_artifact(raw_response)

        return AgentResponse(
            answer=raw_response, citations=retrieval.citations, artifact_type=art_type,
            artifact_title=art_title, artifact_content=art_content, model_used=self.llm_client.model_name, is_grounded=True
        )
