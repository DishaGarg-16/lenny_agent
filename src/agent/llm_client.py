...
class OllamaLLMClient(BaseLLMClient):
    ...
    def __init__(self, base_url: str = None, model: str = None, timeout: float = 45.0):
...
class CloudLLMClient(BaseLLMClient):
    ...
    async def generate(self, system_prompt: str, user_prompt: str, temperature: float = 0.3) -> str:
        if not self.api_key:
            raise ValueError(f"Missing API key for cloud provider '{self.provider}'. Set {self.provider.upper()}_API_KEY in .env")
        if self.provider == "anthropic":
            headers = {"x-api-key": self.api_key, "anthropic-version": "2023-06-01", "content-type": "application/json"}
            payload = {"model": self.model, "max_tokens": 4096, "system": system_prompt, "messages": [{"role": "user", "content": user_prompt}], "temperature": temperature}
            ...
            payload = {"model": self.model, "messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": user_prompt}], "temperature": temperature}
            async with httpx.AsyncClient(timeout=45.0) as client:
...
def get_llm_client(provider_override: Optional[str] = None) -> BaseLLMClient:
    """Factory function returning LLM client (defaults to local Ollama)."""
