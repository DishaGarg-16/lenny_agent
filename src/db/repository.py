...
class ChatRepository:
    ...
    async def get_session(self, session_id: str) -> Optional[SessionModel]:
        """Retrieves a session by ID with its messages and citations pre-loaded."""
        stmt = (
            select(SessionModel)
    ...
    async def add_message(
        ...
        await self.session.flush()  # Generate message.id
        ...
        if citations:
            for cit in citations:
                citation_obj = CitationModel(
                    ...
                    guest=cit.get("guest", ""),
                    timestamp_str=cit.get("timestamp_str", ""),
                    snippet=cit.get("snippet", ""),
                    similarity_score=float(cit.get("similarity_score", 0.0)),
